...
class API_Access:
    ...
    def gain_access(self):
        url = "https://www.copart.com/public/data/reCaptcha/secretKey"
        payload = {'Authorization': 'Basicbase64(user:password)'}
        headers = {
        ...
        s = requests.Session()
        response = s.get(url, data=payload, headers=headers)
        # print(response)
        return response
